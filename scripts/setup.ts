...
async function setupWhatsApp(env: Record<string, string>): Promise<Record<string, string>> {
  banner("WhatsApp (Baileys transport)");
  console.log(
    "Boop talks WhatsApp via embedded Baileys. The first `npm run dev`\n" +
    "will print a QR code in the server log — scan it with the WhatsApp app\n" +
    "on the phone that hosts the agent's number.\n"
  );
  ...
  const answers = await prompts([
    {
      type: "text",
      name: "agentNumber",
      message: "Agent's WhatsApp number (E.164, e.g. +16282809557):",
      initial: env.WHATSAPP_AGENT_NUMBER ?? "",
    },
    {
      type: "text",
      name: "allowedNumbers",
      message: "Allowed sender numbers (comma-separated E.164):",
      initial: env.WHATSAPP_ALLOWED_NUMBERS ?? "",
    },
  ]);

  return {
    ...env,
    WHATSAPP_AGENT_NUMBER: answers.agentNumber ?? env.WHATSAPP_AGENT_NUMBER ?? "",
    WHATSAPP_ALLOWED_NUMBERS: answers.allowedNumbers ?? env.WHATSAPP_ALLOWED_NUMBERS ?? "",
    WHATSAPP_SESSION_DIR: env.WHATSAPP_SESSION_DIR ?? "./auth_info_baileys",
  };
}
...
async function main() {
  ...
  console.log(`
What this does:
  1. Configures your WhatsApp number (Baileys transport — QR scan on first run)
...
`);
  ...
  let existing = readEnv(ENV_PATH);
  existing = await setupWhatsApp(existing);
  writeEnv(ENV_PATH, existing);
...
`);
  ...
  banner("Tunnel — public URL for the WhatsApp webhook");
  console.log(`
ngrok's FREE plan gives you a NEW public URL every restart. For a stable URL, pick one of:
...
⚠ ngrok's FREE plan gives you a NEW URL every restart.  For anything beyond a demo,
  use a stable URL:
...
WhatsApp QR scan (first run only):

  1. Start the server with \`npm run dev\`.
  2. Watch the server log for a QR code.
  3. Open WhatsApp on your agent's phone → Linked Devices → Link a Device.
  4. Scan the QR code. Done — session is saved to WHATSAPP_SESSION_DIR.

Test it:
  • Open http://localhost:5173 for the debug dashboard (Chat tab works
    without a WhatsApp connection).
  • Or send a message from an allowed number to your agent's WhatsApp number.
